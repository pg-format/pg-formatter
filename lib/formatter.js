...
function formatGraph({ lines, comments }, delim, sep) {
  ...
  lines.forEach((line) => {
    if (line.node) {
      declaredNodes.add(JSON.stringify(line.node.id));
  ...
  connectedNodes.forEach((id) => {
    if (!declaredNodes.has(JSON.stringify(id))) {
      formatNode({ id, labels: [], properties: [], pos: { start: 0, end: 0 } }, delim);
      declaredNodes.add(JSON.stringify(id));
    }
  });
