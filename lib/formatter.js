...
function formatGraph({ nodes, edges }, delim, sep) {
  return [...nodes.map((node) => formatNode(node, delim)), ...edges.map((edge) => formatEdge(edge, delim))].join(sep + '\n');
}

function formatNode({ id, labels, properties }, delim) {
  return [formatElement(id), ...labels.map(formatLabel), ...properties.map(formatProperty)].join(delim);
}

function formatEdge({ from, to, direction, labels, properties }, delim) {
  return [`${formatElement(from)} ${direction} ${formatElement(to)}`, ...labels.map(formatLabel), ...properties.map(formatProperty)].join(delim);
}
